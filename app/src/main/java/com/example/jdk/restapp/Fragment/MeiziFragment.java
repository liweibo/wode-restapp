...
public class MeiziFragment extends BaseFragment {
    ...
    public void getData(final int page) {
        setSubscriber(page,false);//进入界面 setSubscriber(1,false);
    }
    public void setSubscriber(final int page,boolean isRefresh){
        ...
        isCache=false;//加载更多时 是不需要做缓存的。加载更多时也不需要刷新，即不需要isRefresh为真。
    }
    @Override
    public void setUserVisibleHint(boolean isVisibleToUser) {//Fragment的懒加载
        super.setUserVisibleHint(isVisibleToUser);
        if(isVisibleToUser) {
            RequestData.getInstance(mContext).setSHProgressinterface(this);//this是用来调用该接口中的方法的，this是MeiziFragment实例对象
        }
