...
public class RequestData {
    ...
    public void setSHProgressinterface(shProgressinterface shProgressinterface) {//setSHProgressinterface
        //用来做一个开关而已 具体的回调实现在BaseFragment
    ...
    public void requestMeiziData(final Observable o1, final Observable o2, final RecyclerView mRecyclerView,
                                 final int page, final List meiziList, final boolean isFirst,
                                 final boolean isShake, boolean isCache) {
    ...
    public void initListener(RecyclerViewDataAdapter mRecyclerViewDataAdapter) {
        ...
        mRecyclerViewDataAdapter.setRecyclerViewItemOnClickListener(
                new RecyclerViewDataAdapter.recyclerViewDataBindingItemOnClickListener() {
                    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
                    @Override
                    public void recyclerViewDataBindingItemOnClick(String url, String desc,
                                                                   String who, Date PublishedAt, String type,
                                                                   int position, int clickPosition) {
                        if (clickPosition == 0) {
                            Intent intent = new Intent();
                            intent.setClass(mContext, ShowWebViewActivity.class);
                            URLTableData urlTableData = new URLTableData(SplitUtils.splitWithComma(url)[0], who,
                                    SplitUtils.splitWithComma(desc)[0], PublishedAt);
